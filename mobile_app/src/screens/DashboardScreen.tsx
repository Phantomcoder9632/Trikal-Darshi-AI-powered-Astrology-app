...
export default function DashboardScreen() {
  ...
  // ── Background pre-generation progress polling ──────────────────────────
  useEffect(() => {
    ...
    poll();
    progressTimer.current = setInterval(poll, 4000);
    return () => {
      cancelled = true;
      ...
      >
        ...
        <View style={styles.identityStrip}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={[typeScale.mono, { color: colors.textFaint, marginTop: 2 }]}>
          ...
          <Card style={{ padding: 16 }}>
            ...
            <View style={{ marginTop: 12, borderTopWidth: 1, borderTopColor: 'rgba(232,213,167,0.7)', paddingTop: 12 }}>
              {interpText ? (
                ...
                <View style={{ gap: 10 }}>
                  <Text style={[typeScale.label, { color: colors.indigo }]}>CHANNELING PARASHARI SHASTRAS…</Text>
                  {[0, 1, 2, 3, 4].map((i) => (
...
const styles = StyleSheet.create({
  ...
  chipCell: { width: '48.5%', flexGrow: 1 },
  progressPill: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 5,
