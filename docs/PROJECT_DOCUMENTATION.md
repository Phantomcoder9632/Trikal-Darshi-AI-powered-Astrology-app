...
6. **Mobile Streaming Client (`mobile_app/src/services/api.ts`)**:
   - Study how React Native's `fetch()` + `ReadableStream` reader consumes the same FastAPI chunked streams as the web client, with an `AbortController` wired to screen lifecycle (not chunk re-renders).

---

## 18. Mobile App (`mobile_app/`) ⬅ NEW

A native-feeling **React Native (Expo SDK 54) + TypeScript** client lives in `mobile_app/`, replicating the web app's parchment + temple indigo + vedic gold design system.

### Screen flow

```
Launch ──▶ Splash (Navagraha Convergence — 9 grahas orbit a golden bindu,
                  7-star constellation progress, rotating status lines)
      ...
      ──▶ Dashboard (cosmic summary · Kundali SVG chart · 11 streaming chapters)
...
| Area | Detail |
| :--- | :--- |
| **Auth** | JWT stored in `expo-secure-store`; axios interceptor injects the bearer token; any 401 triggers a central forced logout (mirrors web behavior); **Google sign-in** via browser flow → backend `/auth/google` |
| **Charts** | North-Indian diamond `react-native-svg` renderer with whole-sign houses and dignity coloring (Exalted/Debilitated/Own/Retro), 9-varga selector, and **tappable planets** — each glyph opens a detail popup (sign, degree °′″, nakshatra + lord, motion, dignity, Sanskrit name, classical lore) |
| **Today screen** | Live Panchanga (tithi, nakshatra + pada, yoga, karana, Moon phase, weekday lord) computed on-device from `GET /chart/gochar`; the nine grahas' current positions; "For Your Chart" overlay mapping transit Moon/Jupiter/Saturn to natal houses + one-tap streaming AI daily reading |
| **Haptics** | Shared tactile vocabulary (`services/haptics.ts`): light ticks on tab/varga/planet taps, medium thud on primary buttons, success haptic on login & stream completion, warning buzz on errors |
| **Friendly errors** | `services/errors.ts` translates raw network/API failures into kind human sentences (cold-start, timeout, offline, rate-limit) |
| **Branding** | Golden ✦-on-parchment app icon, Android adaptive icon and native splash logo — generated by `scripts/gen-icon.py` and wired through `app.json` + `expo prebuild` |
| **Streaming** | Chapters (`POST /interpret/{id}/{tab}`) and chat (`POST /chat`) stream token-by-token over `fetch()` + `ReadableStream`; tab switches never abort an in-flight chapter |
| **Offline** | Charts + interpretations cached in AsyncStorage; backend-unreachable states show real cached data behind an explicit offline banner — never fabricated readings |
| **Progress** | `GET /progress/{id}` polled every 4s for the background pre-generation pill (completed/total chapters) |
...
```The production API URL is configured in `mobile_app/app.json` (`extra.apiUrl`). All builds — including Expo Go development — connect directly to the Hugging Face Space by default; set `extra.useLocalBackend: true` in `app.json` to point Expo Go at a local FastAPI server on the Metro host (port 8000) instead.
---

## 19. AI Plain-Language Contract & Security Hardening ⬅ NEW

### The Simple Language Contract (7th-standard rule)

User feedback: AI interpretations were too rigid and jargon-heavy for people new
to astrology. All generation now enforces a plain-language style contract:

| Where | What |
| :--- | :--- |
| `services/ai_prompts.py → PLAIN_LANGUAGE_STYLE` | Prepended to **all 11 tab prompts** — every technical term explained on use, real-life meaning first, "👉 In plain words:" summaries, planet personalities (Guru = kind teacher, Shani = strict headmaster…), no doom language, 7th-standard reading level |
| `services/ai_prompts.py` language mandates | The same simplicity rules ride along inside the Hindi and Bengali mandates |
| `rag/pipeline.py → SYSTEM_PROMPT` | Matching style block for the RAG generation path |
| `rag/pipeline.py → _CHAT_SYSTEM_PROMPT` | Chat was already plain-language; boundary refusals softened to be warm and human ("outside my starry domain 😊") |

Also fixed: the hard-coded "Now (June 2026)" transit line in prompts is replaced
by **live** Swiss-Ephemeris gochar data read from each chart (with computed-at
stamp and retrograde marks) — no more stale-transit hallucinations. Removed a
stray "This is a North Indian Bengali man's chart" line that wrongly biased
every user's reading.

### Security hardening (backend)

| Fix | File | Detail |
| :--- | :--- | :--- |
| JWT secret enforcement | `main.py` | Production refuses to boot with missing/short (<32 chars) `JWT_SECRET` |
| Security headers | `main.py` | `X-Frame-Options: DENY`, `nosniff`, `Referrer-Policy`, `Permissions-Policy`, conservative CSP on every response |
| Password hashing upgrade | `routes/auth.py` | PBKDF2 100k → **600k iterations** (OWASP 2024); legacy hashes verify + transparently re-hash on login |
| Timing-safe login | `routes/auth.py` | Dummy-hash burn on unknown accounts — no account enumeration via response timing |
| JWT claims | `routes/auth.py` | Tokens now carry `iat` + `jti` (unique ID) for future revocation support |
| Chart-generation rate limit | `routes/chart.py` | **6 charts/hour/IP** (was unlimited — each generation triggers LLM pregen costs) |
| Gochar rate limit | `routes/chart.py` | 30/min/IP on the public ephemeris endpoint |
| Geocode hardening | `routes/geocode.py` | 20/min/IP rate limit + 120-char input cap before cache/OSM calls |
| Chat payload caps | `routes/chat.py` | Message ≤ 2000 chars (non-empty), history ≤ 20 turns × 4000 chars — blocks token-stuffing cost attacks |
| Env hygiene | `.env.example` | Documents JWT secret generation, CORS production policy, never-commit warnings |

Verified: `python -m py_compile` passes on all changed files; validators and
constants confirmed via AST checks. Deploy note: set `APP_ENV=production`,
a 32+ char `JWT_SECRET`, and explicit `CORS_ORIGINS`.
