...
export default function ProfileScreen() {
  ...
  const handleDelete = (chart: ChartSummary) => {
    Alert.alert(
      `Delete ${chart.full_name}'s chart?`,
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await deleteChart(chart.chart_id || chart.id!);
              setCharts((prev) => prev.filter((c) => (c.chart_id || c.id) !== (chart.chart_id || chart.id)));
            ...
            return (
              <Card key={chartId} style={{ padding: 14, marginBottom: 12, borderLeftWidth: isPrimary ? 4 : 1.5, borderLeftColor: isPrimary ? colors.gold : 'rgba(217,166,60,0.45)' }}>
                <View style={{ flexDirection: 'row', gap: 12 }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      <Text style={[typeScale.headlineSm, { color: colors.ink }]} numberOfLines={1}>
                    ...
                    <Text style={[typeScale.mono, { color: colors.textFaint, marginTop: 2, fontSize: 10.5 }]}>
