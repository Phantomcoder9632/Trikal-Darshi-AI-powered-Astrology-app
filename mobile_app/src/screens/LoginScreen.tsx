...
// Same Google client ID the website uses (audience accepted by the backend's
// ID-token verification strategy).
const GOOGLE_WEB_CLIENT_ID = '2126001520-61p5dqr243dg1jjj9obrahhcifod1prv.apps.googleusercontent.com';
// Android-type OAuth client (created in Google Cloud Console with the APK's
// package name + SHA-1). Required for installed-APK sign-in: Google rejects
// custom-scheme redirects for WEB clients (Error 400 invalid_request).
const GOOGLE_ANDROID_CLIENT_ID = 'PASTE_ANDROID_CLIENT_ID_HERE.apps.googleusercontent.com';
...
export default function LoginScreen() {
  ...
  // responseType 'id_token' gives us a JWT the backend verifies directly with
  // audience = GOOGLE_WEB_CLIENT_ID (Strategy 1 in routes/auth.py).
  const [request, response, promptAsync] = Google.useIdTokenAuthRequest({
    clientId: GOOGLE_WEB_CLIENT_ID,
    // On an installed Android build expo-auth-session picks androidClientId
    // and redirects via the app's custom scheme — which only Android-type
    // clients allow.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Auth state flip in AuthContext swaps the navigator to the app stack.
  ...
  const onGooglePress = () => {
    setError('');
    setGoogleBusy(true);
    promptAsync().catch(() => setGoogleBusy(false));
  };
