 ...
 */
export default function KundaliChart({ ascendantSignNum, planets, size = 320 }: KundaliChartProps) {
  ...
  for (let h = 1; h <= 12; h++) {
    houseSigns[h] = ((asc - 1 + (h - 1)) % 12) + 1;
  }

  const planetsByHouse: Record<number, Array<{ planet: PlanetRef; glyph: PlanetGlyph; y: number }>> = {};
  ...
  planets.forEach((p) => {
    ...
    planetsByHouse[houseNum].push({ planet: p, glyph: { name: p.name, abbr: SANSKRIT_ABBRS[p.name] ?? p.name.slice(0, 2), ...style }, y: 0 });
  });
              ...
              {selected && (
                <>
                  ...
                  <Pressable style={styles.closeBtn} onPress={() => setSelected(null)}>
                    ...
                    <Text style={styles.closeStar}> ✦</Text>
                  </Pressable>
                </>
              )}
            </View>
...
/** Tappable planet glyph: invisible rect hit-area over visible text. */
